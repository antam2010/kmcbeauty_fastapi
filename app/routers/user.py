...
from app.repositories.user import create_user, select_user
...
@router.get("/{user_email}", response_model=UserResponse, summary="사용자 조회", description="사용자 정보를 조회합니다.")
def get_user(user_email: str, db: Session = Depends(get_db)):
    return select_user(db, user_email)

@router.put("/{user_email}", response_model=UserResponse, summary="사용자 수정", description="사용자 정보를 수정합니다.")
