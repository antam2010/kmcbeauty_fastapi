...
class TreatmentMenu(Base, SoftDeleteMixin, TimestampMixin):
    ...
    #            문자열로 이어붙여 SQLAlchemy 가 정상 파싱했다(깨지지 않았음). 동작 변화
    #            없이 가독성을 위해 callable(lambda) + and_()/.is_(None) 형태로 교체했다.
    #            lambda 지연 평가로 mapper 구성 시점에 매핑 클래스를 참조한다
    #            (treatment_menu -> treatment_menu_detail 은 단방향 import 로 순환 없음).
